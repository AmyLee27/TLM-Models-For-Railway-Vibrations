...
if(1) %computation?
    ...
    n2_s=5;                                         %<-- substrate: number vertical elements (mesh)
...
%         [uzk]=greenfunction(k1,cinf,c,nodes1,elements1,soil_prop_upper,soil_prop_inf);
% 
%         Ks = stiffvk(k1,width_t*2,uzk); % calculate the equivalent stiffness by eqn(5) in Costa's paper
...
%                     Kts = Kts+KtEq;
%                 case 3
%                     A = [0 0 0; 0 0 0;0 0 Ks(iWave)];                                         % Original Matrix (Created)
%                     N = numel(zeroNod);                                                  % Number Of Times To Repeat
%                     Ar = repmat(A, 1, N);                                   % Repeat Matrix
%                     Ac = mat2cell(Ar, size(A,1), repmat(size(A,2),1,N));    % Create Cell Array Of Orignal Repeated Matrix
%                     Out = blkdiag(Ac{:});                                    % Desired Result
...
%             duy(1:1:end-3,iWave,iFreq)=-sqrt(-1)*ky*u2;
    ...
    figure
    plot(1:81, abs(u(1464:3:1464+81*3-3)),1:81,abs(u(1465:3:1464+81*3-2)),1:81,abs(u(1466:3:1464+81*3-1 )))

...
if(1) %computation?
    ...
    % SAMPLING
    % frequency sampling
    frequency=10;                           %<-- Receivers: Frequency sampling
...
%             duy(1:1:end-3,iWave,iFreq)=-sqrt(-1)*ky*u2;
    ...
    Y=feNod(:,3);
    tri=delaunay(X,Y);
    trisurf(tri,X,Y,abs(uf(3:3:end)));
    shading('interp')
    view([0 -90]);
    hold on
    plot(X,Y,'ok')
    ...
    plot(frequency,abs(squeeze(sigma(4,inode,iElts,:))),'--b')
    plot(frequency,abs(squeeze(sigma(5,inode,iElts,:))),'--r')
    plot(frequency,abs(squeeze(sigma(6,inode,iElts,:))),'--k')
    xlabel('Frequency (Hz)')

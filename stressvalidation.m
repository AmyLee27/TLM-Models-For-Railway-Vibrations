...
if(1) %computation?
    ...
    %TRACK DATA
    %--------------------
    width_t=4/2;                                %<-- track: gauge [m]
    
    bl_s=4;                                         %<-- substrate: width (bottom) [m]
    h_s=0.05;                                       %<-- substrate: height [m]
    % slope_s=30;
    ...
    rho_s=2280;                                     %<-- substrate: density [kg/m^3]
    D_s=0.00;                                          %<-- substrate: damping ratio
    Ezz_s=Ezz_s*(1+sqrt(-1)*D_s);                   %<-- substrate: complex stiffness [N/m^2]
    
    
    bl_a=4;                                         %<-- substrate: width (bottom) [m]
    bu_a=4;
    n1_a=round(n1_s*bl_a/bu_s);                    %<-- asphalt: number horizontal elements (mesh) [pair]
    if(mod(n1_a,2));n1_a=n1_a+1;end
    bl_a=bu_s/n1_s*n1_a;
    bu_a=bl_a;
    Ezz_a=30000E6;                                      %<-- asphalt: Young's modulus [N/m^2]
    nu_a=0.3;                                      %<-- asphalt: Poisson's ratio [-]
    rho_a=2280;                                     %<-- asphalt: density [kg/m^3]
    D_a=0.0;                                       %<-- asphalt: damping ratio
    Ezz_a=Ezz_a*(1+sqrt(-1)*D_a);                   %<-- asphalt: complex stiffness [N/m^2]
    h_a=0.25;                                       %<-- asphalt: height [m]
    n2_a=5;                                         %<-- substrate: number vertical elements (mesh)
    
    h_b=0.0;                                       %<-- ballast: height [m]
    ...
    [feNod,feElt,feTyp,feMat,feSec,T1,t1,dofmaster,P]=asphalt_track_8nodes1(width_t,bl_s,h_s,bu_s,z0_s,n1_s,n2_s,h_b,bu_b,bl_b,n2_b,n1_b,n2_a,n1_a,h_a,rho_s,Ezz_s,nu_s,...
    ...
    % SAMPLING
    % frequency sampling
    frequency=0;                           %<-- Receivers: Frequency sampling
...
%     duy=zeros(nFeDof+3,nWave,nFreq);
    ...
    u2=zeros(nFeDof,1);
    uzk_nFreq=zeros(length(py),length(py),nFreq);
    Stress1=zeros(length(py),length(py),length(soil_prop_upper(:,1)),nFreq);
    Stress2=zeros(length(py),length(py),length(soil_prop_upper(:,1)),nFreq);
    Stress3=zeros(length(py),length(py),length(soil_prop_upper(:,1)),nFreq);
    Stress4=zeros(length(py),length(py),length(soil_prop_upper(:,1)),nFreq);
    Stress5=zeros(length(py),length(py),length(soil_prop_upper(:,1)),nFreq);
    Stress6=zeros(length(py),length(py),length(soil_prop_upper(:,1)),nFreq);
...
%         [uzk] = greenfunction(py.*omega(iFreq),cinf,c,nodes1,elements1,soil_prop_upper,soil_prop_inf); % calculate the greens function
        ...
        uzk_nFreq(:,:,iFreq)=uzk;
        Stress1(:,:,:,iFreq)=stress1;
        Stress2(:,:,:,iFreq)=stress2;
        Stress3(:,:,:,iFreq)=stress3;
        Stress4(:,:,:,iFreq)=stress4;
        Stress5(:,:,:,iFreq)=stress5;
        Stress6(:,:,:,iFreq)=stress6;

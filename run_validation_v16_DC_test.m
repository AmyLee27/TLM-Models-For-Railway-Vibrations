...
if(1) %computation?
    %% PROPERTIES
    %SOIL DATA
    ksoilv=1e20;                                     %<-- soil: complex stiffness [N/m^2]   1e20~infinite stiffness!
    ksoilh=1e20;                                     %<-- soil: complex stiffness [N/m^2]   1e20~infinite stiffness!
    ksoill=1e20;
    Dsoil=0.05;
    ksoilv=ksoilv*(1+sqrt(-1)*Dsoil);
    ksoilh=ksoilh*(1+sqrt(-1)*Dsoil);
    ksoill=ksoill*(1+sqrt(-1)*Dsoil);
    ...
    D_s=0.05;                                          %<-- substrate: damping ratio
    ...
    D_a=0.05;                                       %<-- asphalt: damping ratio
    ...
    D_b=0.06;                                          %<-- ballast: damping ratio
    ...
    rho_rp=500;                                       %<--  rail pad: density [kg/m^3]
    
    D_rp=0.05;                                    %<-- rail pad: damping [Ns^2/m]
    Ezz_rp=Ezz_rp*(1+sqrt(-1)*D_rp);                   %<-- rail pad: complex stiffness [N/m^2]
    
    Ezz_rp=Ezz_rp*b_rp/d_sl;                                   %<-- rail pad: stiffness [N/m/m]
    kzz_rp=kzz_rp/d_sl;                                   %<-- rail pad: stiffness [N/m/m]
    ...
    % set paramters for displacement and time
    ...
    P1=T1*P;
    P2=zeros(length(P1)+dofs,1); % include force in the track and also the soil (3 means 3 dofs from the soil)
    P2(1:length(P1))=P1;
    %displacement boundary condition
    P2(end+1-dofs:end)=[];  % the last three dofs means the soil
    
    % SAMPLING
    % frequency sampling
    frequency=1;                           %<-- Receivers: Frequency sampling
...
%     duy=zeros(nFeDof+3,nWave,nFreq);
    u=zeros(nFeDof+dofs,nWave);
    duy=zeros(nFeDof+dofs,nWave);
    unew = zeros(nFeDof+dofs,nWave,nFreq);
    duynew=zeros(nFeDof+dofs,nWave,nFreq);
    u2=zeros(nFeDof,1);
    ...
    parfor iFreq=1:nFreq
        ...
        for iWave=1:nWave
            ...
            %Soil stiffness
            Kts=zeros(size(Kt,1)+dofs,size(Kt,2)+dofs);
            Kts(1:size(Kt,1),1:size(Kt,2))=Kt;
            
            switch dofs
                case 1
                    Kts(end,end)=ksoilv;
                    Kts(dofmaster,dofmaster)=Kts(dofmaster,dofmaster)+ksoilv;
                    Kts(end,dofmaster)=Kts(end,dofmaster)-ksoilv;
                    Kts(dofmaster,end)=Kts(end,dofmaster)-ksoilv;
                    Kts = sparse(Kts);
                    
                case 2
                    Kts(end-1,end-1)=ksoilh;
                    Kts(end,end)=ksoilv;
                    
                    Kts((dofmaster-1)*2+1,(dofmaster-1)*2+1)=Kts((dofmaster-1)*2+1,(dofmaster-1)*2+1)+ksoilh;
                    Kts((dofmaster-1)*2+2,(dofmaster-1)*2+2)=Kts((dofmaster-1)*2+2,(dofmaster-1)*2+2)+ksoilv;
                    
                    Kts(end-1,(dofmaster-1)*2+1)=Kts(end-1,(dofmaster-1)*2+1)-ksoilh;
                    Kts((dofmaster-1)*2+1,end-1)=Kts((dofmaster-1)*2+1,end-1)-ksoilh;
                    Kts(end,(dofmaster-1)*2+2)=Kts(end,(dofmaster-1)*2+2)-ksoilv;
                    Kts((dofmaster-1)*2+2,end)=Kts((dofmaster-1)*2+2,end)-ksoilv;
            
                case 3
                    
                    Kts(end-2,end-2)=ksoilh;
                    Kts(end-1,end-1)=ksoill;
                    Kts(end,end)=ksoilv;
            
                    Kts((dofmaster-1)*3+1,(dofmaster-1)*3+1)=Kts((dofmaster-1)*3+1,(dofmaster-1)*3+1)+ksoilh;
                    Kts((dofmaster-1)*3+2,(dofmaster-1)*3+2)=Kts((dofmaster-1)*3+2,(dofmaster-1)*3+2)+ksoill;
                    Kts((dofmaster-1)*3+3,(dofmaster-1)*3+3)=Kts((dofmaster-1)*3+3,(dofmaster-1)*3+3)+ksoilv;
           
                    Kts(end-2,(dofmaster-1)*3+1)=Kts(end-2,(dofmaster-1)*3+1)-ksoilh;
                    Kts((dofmaster-1)*3+1,end-2)=Kts((dofmaster-1)*3+1,end-2)-ksoilh;
                    Kts(end-1,(dofmaster-1)*3+2)=Kts(end-1,(dofmaster-1)*3+2)-ksoill;
                    Kts((dofmaster-1)*3+2,end-1)=Kts((dofmaster-1)*3+2,end-1)-ksoill;          
                    Kts(end,(dofmaster-1)*3+2)=Kts(end,(dofmaster-1)*3+2)-ksoilv;
                    Kts((dofmaster-1)*3+2,end)=Kts((dofmaster-1)*3+2,end)-ksoilv;
            end
            
            Kts = sparse(Kts);
            
            %displacement boundary condition
            Kts(:,end+1-dofs:end)=[];  % depends on dofs
            Kts(end+1-dofs:end,:)=[];
            
            u2=Kts\P2;
%             u(1:1:end-3,iWave,iFreq)=u2;% depends on dofs
            u(1:1:end-dofs,iWave) = u2;
%             duy(1:1:end-3,iWave,iFreq)=-sqrt(-1)*ky*u2;
            duy(1:1:end-dofs,iWave)=-sqrt(-1)*ky*u2;
        end
    ...
    uf=zeros(nFeDof+dofs,nFreq);
    duyf=zeros(nFeDof+dofs,nFreq);
    duyf2=zeros(nFeDof+dofs,nFreq);
    ...
    figure
    plot(frequency,abs(uf(end-2*dofs,:)))
    ...
    pressure = sigman(3,zeroNod,:);
    v=diff(feNod(zeroNod,2));

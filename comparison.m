...
if(1) %computation?
    ...
    D_a=0.04;                                       %<-- asphalt: damping ratio
    ...
    Ezz_rp=38.1e6; %kzz_rp*h_rp/l_rp/b_rp;                                   %<-- rail pad: Young's modulus [N/m^2]
    nu_rp=0.49;                                        %<--  rail pad: Poisson's ratio [-]
    rho_rp=1000;                                       %<--  rail pad: density [kg/m^3]
    
    D_rp=0.021;                                    %<-- rail pad: damping [Ns^2/m]
...
%             duy(1:1:end-3,iWave,iFreq)=-sqrt(-1)*ky*u2;
    ...
    figure
    plot(frequency,abs(uf(end-2*dofs,:))) % plot displacement on rail only
    hold on

...
if(1) %computation?
    ...
    % SAMPLING
    % frequency sampling
    frequency=1:5:400;                           %<-- Receivers: Frequency sampling
...
%     duy=zeros(nFeDof+3,nWave,nFreq);
    ...
    Stress1=zeros(length(py),length(py),length(py),nFreq);
    Stress2=zeros(length(py),length(py),length(py),nFreq);
    Stress3=zeros(length(py),length(py),length(py),nFreq);
    Stress4=zeros(length(py),length(py),length(py),nFreq);
    Stress5=zeros(length(py),length(py),length(py),nFreq);
    Stress6=zeros(length(py),length(py),length(py),nFreq);
...
%         [uzk] = greenfunction(py.*omega(iFreq),cinf,c,nodes1,elements1,soil_prop_upper,soil_prop_inf); % calculate the greens function
        [uzk,stress1,stress2,stress3,stress4,stress5,stress6]=greenfunction_stress2(py.*omega(iFreq),cinf,c,nodes1,elements1,soil_prop_upper,soil_prop_inf);
        uzk_nFreq(:,:,iFreq)=uzk;
